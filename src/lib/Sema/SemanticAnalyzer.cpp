...
SemanticAnalyzer::SemanticAnalyzer() { scopes.clear(); }
SemanticAnalyzer::Symbol::Symbol(llvm::StringRef name, Kind kind, Type type,
                                 std::shared_ptr<AstNode> node)
    : name(name), kind(kind), type(type), node(node) {}

llvm::StringRef SemanticAnalyzer::Symbol::getName() const { return name; }

SemanticAnalyzer::Symbol::Kind SemanticAnalyzer::Symbol::getKind() const {
  return kind;
}

Type SemanticAnalyzer::Symbol::getType() const { return type; }

std::shared_ptr<AstNode> SemanticAnalyzer::Symbol::getNode() { return node; }
...
bool SemanticAnalyzer::Scope::isLoopScope() { return isLoop; }

void SemanticAnalyzer::Scope::setLoop(bool isLoop) { this->isLoop = isLoop; }

SemanticAnalyzer::Symbol *
SemanticAnalyzer::Scope::lookup(llvm::StringRef name) {
...
bool SemanticAnalyzer::checkType(std::shared_ptr<Type> type) { return true; }

bool SemanticAnalyzer::checkType(std::shared_ptr<Type> type1,
                                 std::shared_ptr<Type> type2) {
  return *type1 == *type2;
}

std::shared_ptr<Type> SemanticAnalyzer::getType() const { return returnType; }
...
void SemanticAnalyzer::visit(Constant &node) { node.visitChildren(*this); }
...
void SemanticAnalyzer::visit(DeclStmt &node) { node.visitChildren(*this); }
...
void SemanticAnalyzer::visit(FuncDecl &node) {
  ...
  node.visitChildren(*this);
  insertSymbol(Symbol(node.getName(), Symbol::Kind::Function,
                      *node.getReturnType(), std::make_shared<FuncDecl>(node)));
}
...
void SemanticAnalyzer::visit(NullStmt &node) { node.visitChildren(*this); }
...
bool SemanticAnalyzer::hasErrorOccurred() { return hasError; }
